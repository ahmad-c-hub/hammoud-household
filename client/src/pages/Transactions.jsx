...
const selectCls = 'border border-slate-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full';
...
export default function Transactions() {
  ...
  function exportToExcel() {
    const rows = sorted.map(e => ({
      Date: e.date, Person: e.user_name, Category: e.category,
      Amount: (e.amount_cents / 100).toFixed(2), Note: e.note || '',
    }));
  ...
  const hasFilters = filters.start || filters.end || filters.category || filters.user_id;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 mb-4">
        ...
        {hasFilters && (
          <button
            onClick={() => setFilters({ start: '', end: '', category: '', user_id: '' })}
            className="text-sm text-indigo-600 hover:text-indigo-800 underline text-left sm:col-span-2 lg:col-span-4"
          >
            Clear filters
          </button>
        )}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[560px]">
                ...
                <tfoot className="bg-slate-50 border-t border-slate-200">
                  <tr>
                    <td colSpan={3} className="px-4 sm:px-6 py-3 text-xs text-slate-500">
                      {sorted.length} transaction{sorted.length !== 1 ? 's' : ''}
                    </td>
                    <td className="px-4 sm:px-6 py-3 font-bold text-red-500 text-sm whitespace-nowrap">
                      {formatCurrency(sorted.reduce((s, e) => s + e.amount_cents, 0))}
                    </td>
                    <td />
                  </tr>
                </tfoot>
