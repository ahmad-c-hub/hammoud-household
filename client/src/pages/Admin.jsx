        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-6 pt-1">
            <label className="flex items-center gap-2.5 text-sm text-slate-700 cursor-pointer select-none">
              <input
                type="checkbox" checked={form.has_income}
            ...
            <label className="flex items-center gap-2.5 text-sm text-slate-700 cursor-pointer select-none">
              <input
                type="checkbox" checked={form.can_spend}
  ...
  return (
    <div className="space-y-8">
              ...
              {users.map(u => (
                <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-3.5 text-right space-x-4">
                    <button onClick={() => setModal(u)} className="text-xs font-medium text-indigo-600 hover:text-indigo-800 transition-colors">Edit</button>
                    <button onClick={() => handleDeleteUser(u.id)} className="text-xs font-medium text-red-400 hover:text-red-600 transition-colors">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6">
        <h2 className="text-base font-semibold text-slate-700 mb-4">Category Management</h2>
        <form onSubmit={handleAddCategory} className="flex gap-3 mb-5 max-w-sm">
          <input
            type="text"
            placeholder="New category name"
            value={newCat}
            onChange={e => setNewCat(e.target.value)}
            className="flex-1 border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
          />
          <button
